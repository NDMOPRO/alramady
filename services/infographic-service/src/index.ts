...
const logger = winston.createLogger({
  ...
  defaultMeta: { service: 'infographic-service' },
  transports: [
...
app.get('/api/v1/infographic/ready', (_req, res) => {
...
app.use('/api/v1/infographic', infographicRoutes);
app.use('/api/v1/infographic/professional', professionalRoutes);
...
async function bootstrap(): Promise<void> {
  try {
    ...
    app.listen(PORT, () => {
      logger.info(`infographic-service running on port ${PORT}`);
      logger.info(`Health check: http://localhost:${PORT}/health`);
    });
  } catch (error) {
    logger.error('Failed to start infographic-service', { error });
    process.exit(1);
  }
}

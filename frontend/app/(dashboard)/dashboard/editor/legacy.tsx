...
import { buildDashboardEditorWidgetsFromDraft } from '@/lib/workspaces/draft-presets';
import {
...
export default function DashboardEditorPage() {
  ...
  const dashboardDraft = useWorkspaceDraft('dashboard');
  ...
  useEffect(() => {
    if (!dashboardDraft || artifactApplied) return;
    if (dashboardDraft.items.length === 0) return;

    const generated = buildDashboardEditorWidgetsFromDraft(dashboardDraft);
    if (generated.length === 0) return;

    setWidgets(generated);
    setHistory([{ widgets: generated, label: `Artifact ${dashboardDraft.artifactId}` }]);
    setHistoryIndex(0);
    setSelectedId(generated[0]?.id ?? null);
    setArtifactApplied(true);
  }, [dashboardDraft, artifactApplied]);
  ...
  return (
    <div className="flex h-screen flex-col overflow-hidden bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 text-white" dir="rtl">
      ...
      {artifactApplied && dashboardDraft && (
        <div className="mx-4 mt-3 rounded-xl border border-cyan-500/30 bg-cyan-500/10 px-4 py-2 text-xs text-cyan-200">
          تم تهيئة المحرر من حزمة المطابقة {dashboardDraft.artifactId} ({dashboardDraft.items.length} عناصر).
        </div>
      )}

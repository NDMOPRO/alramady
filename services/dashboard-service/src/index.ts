...
app.get('/api/v1/dashboard/ready', (_req, res) => {
...
import autoDashboardRoutes from './routes/auto-dashboard.routes.js';
import advancedModeRoutes from './routes/advanced-mode.routes.js';
import dragElementsRoutes from './routes/drag-elements.routes.js';
...
import fullEditorRoutes from './routes/full-editor.routes.js';
...
app.use('/api/v1/dashboard/advanced', advancedModeRoutes);
app.use('/api/v1/dashboard/drag', dragElementsRoutes);
app.use('/api/v1/dashboard/easy', easyModeRoutes);
app.use('/api/v1/dashboard/simulation', externalSimulationRoutes);
app.use('/api/v1/dashboard/editor', fullEditorRoutes);
...
async function bootstrap(): Promise<void> {
  try {
    ...
    // Start BullMQ worker for heavy dashboard generation jobs
    try {
      const { startAutoDashboardWorker } = await import('./services/auto-dashboard-generator.service.js');
      startAutoDashboardWorker();
      logger.info('Auto-dashboard BullMQ worker started');
    } catch (workerErr) {
      logger.warn('Failed to start auto-dashboard worker, feature will use sync mode', {

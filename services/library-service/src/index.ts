...
const logger = winston.createLogger({
  ...
  defaultMeta: { service: 'library-service' },
  transports: [
...
const limiter = rateLimit({
  ...
  legacyHeaders: false,
  skip: (req) => req.path === '/health' || req.path === '/api/v1/library/ready',
  message: {
    success: false,
...
app.get('/api/v1/library/ready', (_req, res) => {
...
async function bootstrap(): Promise<void> {
  try {
    ...
    logger.error('Failed to start library-service', { error });
    process.exit(1);
  }
}
